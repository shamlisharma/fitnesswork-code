...
const Timer = React.memo(props => {
  ...
  const getTimerStatus = useSelector(
    state => state.workout.workoutDurationCheck,
  );
  useEffect(() => {
  ...
  useLayoutEffect(() => {
    if (appStateVisible === 'background') {
      dispatch({
        type: workoutProgrammeActionTypes.WORKOUT_DURARATION_ACTIVE,
        payload: {
          active: true,
          previousTime: time,
  ...
  const updateDuration = () => {
    dispatch({
      type: workoutProgrammeActionTypes.UPDATE_WORKOUT_DURATION,
      payload: second,
    });
  };
