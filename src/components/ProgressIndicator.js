  ...
  return (
    <TouchableOpacity style={styles.container} onPress={onPress}>
      <View style={{paddingHorizontal: 8}}>
        ...
        />
       <View style={{position:"absolute",paddingHorizontal:23,paddingVertical:15}}> 
         {icon} 
      ...
      <View style={{flex: 1}}>
        ...
        {workoutFromHome ? (
          <Text style={[styles.progressTextStyle,{color:calories?color:null}]}>{getSleepTime()}</Text>
        ) : (
          <Text style={[styles.progressTextStyle]}>
            {`${progressValue}${targetValue ? '/' : ''}`}
            {targetValue ? (
              <Text style={styles.targetTextStyle}>
                {targetValue}
                </Text>
            ) : null}
          </Text>
